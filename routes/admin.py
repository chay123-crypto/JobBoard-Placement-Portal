    ...
    return jsonify(
        {
            ...
            "num_placement_drives": num_placement_drives,
            "num_placed": num_placedstudents,
            "num_applications": num_applications,
            "maxSalary": max_salary,
            "minSalary": min_salary,
            "avgSalary": avg_salary,
            "medianSalary": median_salary,
            "AvgStipend": avg_stipend,
            "MedianStipend": median_stipend,
        }
    )
...
def schedule_interview(interview_id):
    ...
    task = get_reminders.delay(interview.student_id, interview.interview_date, interview.interview_time, interview.company.name)
    cache.clear()
    return jsonify({"status": "success", "task_id": task.id, "message": "Interview reminder queued"}), 201
