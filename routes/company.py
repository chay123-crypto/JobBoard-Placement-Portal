...
from models.models import StudentProfile, CompanyProfile, db, Application, PlacementDrive, Interview
...
def company_dashboard():
    ...
    return jsonify({"company_name": company.name, "website": company.website, "approval_status": company.approval_status, "field": company.field, "location": company.location, "num_applications": num_applications, "num_placement_drives": num_placement_drives, "drives": lis})
...
def invite_interview(application_id):
    try:
        ...
        interview = Interview(student_id=application.student_id, company_id=application.drive.company_id, application_id=application_id, interview_date=data["interview_date"], interview_time=data["interview_time"], location=data["location"], status="scheduled")
        db.session.add(interview)
        db.session.commit()
        cache.clear()
        task = get_reminders.delay(interview.student_id, interview.interview_date, interview.interview_time, interview.company.name, interview.location)
